...
import { UpdatePreparationProgressRequest } from "./job-preparation-plan.types";
...
export class JobAnalysisController {
  ...
  @Patch(":opportunityId/preparation-plan/items/:requirementId")
  updatePreparationProgress(
    @Headers("authorization") authorization: string | undefined,
    @Param("opportunityId") opportunityId: string,
    @Param("requirementId") requirementId: string,
    @Body() body: UpdatePreparationProgressRequest
  ) {
    const user = this.authService.getUserFromAuthorization(authorization);
    return this.jobPreparationPlanService.updateItemStatus(user.id, opportunityId, requirementId, body);
  }
}
